      ...
      <div class="nav-content flex justify-between	items-center max-w-5xl mx-auto">
        ...
        <ul class="m-0 p-0 hidden sm:flex">
          <li v-for="(link,i) in slice.items" :key="i">
            <prismic-link class="hover:bg-gray-200	focus:bg-gray-200	p-3 rounded-xl ml-2" :field="link.link">
              {{ link.label }}
            </prismic-link>
          </li>
        </ul>
        <div class="flex flex-col justify-between	items-end bg-gray-200 py-4 px-3 sm:hidden toggle-btn rounded-2xl cursor-pointer">
